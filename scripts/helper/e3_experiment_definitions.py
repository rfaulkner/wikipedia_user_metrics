...
import user_metrics.etl.log_parser as lp
...
experiments = {
    ...
    'cta4' : {
                ...
                'user_list' : lambda d: [d.__setitem__(row[0],row[1]) for row in dl.Connector(
                    instance = 'slave').execute_SQL('select user_id, min(timestamp) from e3_cta4_server_logs '
                                                    'where self_made=1 group by 1')]
    },
    ...
    'acux2' : {
        ...
        'user_list' : lambda d: [d.__setitem__(row[0],row[1]) for row in dl.Connector(
            instance = 'slave').execute_SQL('select user_id, min(timestamp) from e3_acux2_server_events '
                                            'where self_made=1 group by 1')],
...
experiments['acux3']['user_list'] = lambda d: [d.__setitem__(row[0],row[1]) for row in dl.Connector(
    instance = 'slave').execute_SQL('select user_id, min(timestamp) from e3_acux3_server_events '
                                    'where self_made=1 group by 1')]
