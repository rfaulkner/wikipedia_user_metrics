...
"""
    Implements a simple logistic regression classifier with L2 regularization.  This has been adapted from the
    implementation authored by Daniel Tarlow: ::

            `http://blog.smellthedata.com/2009/06/python-logistic-regression-with-l2.html`

    e.g. ::

    >>> lr = LogisticRegression(x_train=xx, y_train=xx, x_test=xx, y_test=xx, alpha=a)
    >>> lr.train()
    >>> print lr.lik(lr.betas)
"""

__author__ = "Ryan Faulkner <rfaulkner@wikimedia.org>"
__date__ = "December 10th, 2012"
...
class LogisticRegression():
    ...
    def lik(self, betas):
        ...
        for i in range(self.n):
            l += np.log(sigmoid(self.y_train[i] * np.dot(betas, self.x_train[i,:])))
