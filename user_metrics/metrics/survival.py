...
class Survival(um.UserMetric):
    """
        Boolean measure of the retention of editors. Editors are considered
        "surviving" if they continue to participate after t minutes since an
        event (often, the user's registration or first edit).
        ...
        process() produces a nested list that stores in each element:
            ...
            * User ID
            * boolean flag to indicate whether the user met the survival
                criteria

        usage e.g.: ::

            >>> import user_metrics.etl.threshold as t
            >>> for r in t.Threshold().process([13234584]).__iter__():
                print r
            (13234584L, 1)
    ...
    def process(self, user_handle, **kwargs):
        ...
        """
            Wraps the functionality of UserMetric::Threshold by setting the
            `survival` flag in process().
