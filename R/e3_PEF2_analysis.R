...
#
# Ryan Faulkner, October 2nd 2012
#
# Editor Engagement Experiments
#
# Post Edit Feedback - Iteration #2 - Analysis
#
...
# FUNCTION :: pef2.bytes.added.process
#
# Code for PEF-2 analysis - bytes added metric
#
# data - 
# columns - 

pef2.bytes.added.process <- function(data, columns) {

	treatments <- c("control", "historical")
	...
	for (i in columns)
	{
        ...
        for (j in treatments) {
            ...
            if (length(data_lst) > 5000)
              data_lst_shapiro <- data_lst[1:5000]
            else
              data_lst_shapiro <- data_lst
            ...
            print(paste("Shapiro Test for", j, "under", i, sep=" "))
	          print(shapiro.test(log(data_lst_shapiro)))
        ...
        t1 <- new_data[[i]][filter.list.by.regex("historical", new_data$bucket)]
        ...
        filename = paste("/Users/rfaulkner/projects/Analysis/Post-Edit-Feedback/i2/", "PEF2_", i, ".png", sep="")
        ...
        # ERRORBARS
        df <- data.frame(treatment=factor(c("Control", "Confirmation")), bytes_added=c(mean(ctrl), mean(t1)), se=c(sd(ctrl), sd(t1)))
...
# FUNCTION :: pef2.bytes.added.process
#
# Code for PEF-2 analysis bytes added
#

pef2.time.to.threshold.process <- function(data) {
