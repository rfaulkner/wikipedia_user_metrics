...
# FUNCTION :: generate.aggregate.data.frame
#
#   generate.aggregate.data.frame - Transform function.  Takes a data-frame containing the samples from an experiment and 
#
#
#   formatted data frame containing:
#   
#     $x - samples
#     $bucket - treatment

generate.aggregate.data.frame <- function(data, interval=1, treatment_index="bucket", data_index="x", is_count=FALSE)
{
  
  samples <- data[[data_index]]
  treatments <- levels(factor(data[[treatment_index]]))
  treatment_types <- levels(factor(data[[treatment_index]]))
  
  if !(is_count)
    counts <- data.frame()  
  
  # Initialize the data frame output
  bins=seq(min(samples),max(samples),by=interval)
  ret <- data.frame(x=bins + interval/2)
  
  for (i in 1:length(treatment_types)) {
    effect_attribute <- paste("y",i,sep="")
    bucket_attribute <- paste("bucket",i,sep="")
    
    ret[[effect_attribute]] <- rep(0,length(bins)-1)
    ret[[bucket_attribute]] <- treatment_types[i]
    
    if !(is_count) {
      effect_attribute_min <- paste(effect_attribute,"_min")
      effect_attribute_max <- paste(effect_attribute,"_max")
      
      ret[[effect_attribute_min]] <- rep(0,length(bins)-1)
      ret[[effect_attribute_max]] <- rep(0,length(bins)-1)
      
    }
  }
  
  
  # For each sample add it to a bin
  bad_count <- 0
  for (i in 1:length(samples)) {
    ...
    index <- which(treatment_types == t)
    effect_attribute <- paste("y",index,sep="")
    ...
    {  
      bin <- bin - 1
      if (is_count)  
        ret[[effect_attribute]][bin] <- ret[[effect_attribute]][bin] + 1
      else
        ret[[effect_attribute]][bin] <- ret[[effect_attribute]][bin] + s
    }
    else
      # ignore the value, it falls outside of the bins
